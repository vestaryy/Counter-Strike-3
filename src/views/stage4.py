...
class Stage4(arcade.View):
    ...
    def on_show_view(self):
        ...
        self.health = UILabel(text="100",
                              ...
                              width=300,
                              align="left")
        self.goal = UILabel(text="Задача: найти передатчик и выключить его",
                            font_size=20,
                            font_name='SimSun',
        ...
        self.enemies_textures = {
            '1': {'default': arcade.load_texture('../../assets/textures/enemy1pack/defoult.png'),
            ...
            '2': {'default': arcade.load_texture('../../assets/textures/enemy2pack/defoult.png'),
            ...
            '3': {'default': arcade.load_texture('../../assets/textures/enemy3pack/default.png'),
                  'walk1': arcade.load_texture('../../assets/textures/enemy3pack/walk1.png'),
                  'walk2': arcade.load_texture('../../assets/textures/enemy3pack/walk2.png'),
                  'headshot': arcade.load_texture('../../assets/textures/enemy3pack/headshot.png'),
                  'bodyshot': arcade.load_texture('../../assets/textures/enemy3pack/bodyshot.png'),
                  'death': arcade.load_texture('../../assets/textures/enemy3pack/death.png'),
                  'shoot': arcade.load_texture('../../assets/textures/enemy3pack/shoot.png')
                  },
            'B': {'default': arcade.load_texture('../../assets/textures/boss/default.png'),
                  'walk1': arcade.load_texture('../../assets/textures/boss/walk1.png'),
                  'walk2': arcade.load_texture('../../assets/textures/boss/walk2.png'),
                  'headshot': arcade.load_texture('../../assets/textures/boss/headshot.png'),
                  'bodyshot': arcade.load_texture('../../assets/textures/boss/bodyshot.png'),
                  'death': arcade.load_texture('../../assets/textures/enemy3pack/death.png'),
                  'shoot': arcade.load_texture('../../assets/textures/boss/shoot.png')
                  }
                }

        self.enemies_settings = {
            '1': {'can_attack': True, 'can_move': True, 'd': 20},
            '2': {'can_attack': True, 'can_move': True, 'd': 20},
            '3': {'can_attack': True, 'can_move': True, 'd': 20},
            'B': {'can_attack': True, 'can_move': True, 'd': 20},
        ...
        self.wall_textures = {
            ...
            's': arcade.load_texture('../../assets/textures/sky&walls/scan.png'),
        ...
        self.map_width, self.map_height = len(max(self.text_map, key=len)), len(self.text_map)
        self.map_textures = {}
        self.block_map = set()
        ...
        self.final_music = arcade.load_sound('../../assets/sounds/background/59. The Farewell(final).mp3')
        with open('../../data/data.txt', 'r', encoding='utf-8') as f:
            ...
            if len(lines) == 4:
                ...
                self.kalash.patrons, self.kalash.magazines = (int(lines[3].split('/')[0]), int(lines[3].split('/')[1]))
                ...
                if col in self.wall_textures.keys():
                    ...
                    if col != 'B':
                        spr = Enemies(
                            ...
                            True,
                            100,
                            self.enemies_settings[col]['d'],
                            max(self.map_width, self.map_height) * self.block_size
        ...
        self.music_player_bg = arcade.load_sound('../../assets/sounds/background/10. Night 4.mp3').play(0.3, loop=True)
        self.patrons_sound = arcade.load_sound(
            '../../assets/sounds/background/bullet-casing-7mm-bullets-drop-pile-cement_my8iaf4o.mp3')
        self.final_slide_list = arcade.SpriteList()
        self.final_slide_list.append(arcade.Sprite('../../assets/textures/elements/final2.png', 2.1 - SCREEN_SCALE, SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
        ...
        self.floor_tex = self.window.ctx.load_texture(
            '../../assets/textures/sky&walls/textur-gas-kvas-com-tc5o-p-teksturi-pol-laboratorii-5.jpg')
        ...
        self.current_slide = arcade.load_texture('../../assets/textures/sky&walls/light.png')
        self.stat_text_batch = Batch()
        self.all_shoots = arcade.Text('Всего: 100 выстрелов', SCREEN_WIDTH // 2, 50, arcade.color.BLACK,
                                      20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='center')
        self.headshots_shoots = arcade.Text('Голова', SCREEN_WIDTH // 2.8, SCREEN_HEIGHT // 2,
                                      arcade.color.BLACK,
                                      20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='right')
        self.headshots_percent = arcade.Text('Голова', SCREEN_WIDTH // 2.8, SCREEN_HEIGHT // 2 - 100,
                                            arcade.color.BLACK,
                                            20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='right')
        self.bodyshots_percent = arcade.Text('Тело', SCREEN_WIDTH // 1.6, SCREEN_HEIGHT // 4,
                                             arcade.color.BLACK,
                                             20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='left')
        self.bodyshots_shots = arcade.Text('Тело', SCREEN_WIDTH // 1.6, SCREEN_HEIGHT // 4 + 100,
                                             arcade.color.BLACK,
                                             20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='left')
        self.next = arcade.Text('ESC - выйти', SCREEN_WIDTH, SCREEN_HEIGHT,
                                             arcade.color.BLACK,
                                             20, font_name='SimSun', batch=self.stat_text_batch, anchor_x='right', anchor_y='top')
    ...
    def on_draw(self):
        ...
        if self.flag2:
            self.final_slide_list.draw()
            self.stat_text_batch.draw()
            arcade.draw_rect_filled(arcade.rect.LRBT(0,  SCREEN_WIDTH, 0, SCREEN_HEIGHT), (0, 0, 0, self.transparency))
    ...
    def on_update(self, delta_time):
        ...
        if self.flag2:
            if self.transparency > 0:
                self.transparency -= delta_time * 5
            if self.transparency < 0:
                self.transparency = 0
            return
        ...
        if (dx or dy) and self.timer_steps > 0.5:
            ...
            self.timer_steps = 0
        if 1400 < self.player.x < 1500 and 2300 < self.player.y < 2400 and -0.5 < sin(self.player.angle) < 0.5 and cos(self.player.angle) > 0.8:
            self.draw_unlock = 1
        elif 200 < self.player.x < 300 and 200 < self.player.y < 300 and -0.5 < sin(self.player.angle) < 0.5 and cos(self.player.angle) > 0.8 and (300, 200) in self.block_map:
            ...
            if enemy.has_patrons and type(
                ...
                if arcade.key.E in self.player.keys_pressed and self.draw_key:
                    ...
                    if enemy.enemy_type == 'B':
                        self.add_patrons.text = 'Ключ-карта для передатчика'
                        arcade.load_sound('../../assets/sounds/background/game-bonus-french_zkfnabeu.mp3').play()
                        arcade.stop_sound(self.music_player_bg)
                        self.music_player_bg = self.final_music.play(loop=True)

                        self.flag = True

                    elif self.robberied_count == 8:
        ...
        if key == arcade.key.E:
            if self.draw_unlock:
                if (300, 200) in self.block_map and self.draw_unlock == (300, 200):
                    ...
                    with open('../../data/data.txt', 'w', encoding='utf-8'):
                        pass
                    with open('../../data/statistics.txt', 'r', encoding='utf-8') as f:
                        lines = [line.strip() for line in f.readlines()]
                        old_shoot = int(lines[0])
                    ...
                    self.all_shoots.text = f'Всего выстрелов: {self.shoot}. Процент попадания: {round((self.headshot + self.bodyshot) / self.shoot * 100, 2)}%'
                    self.bodyshots_shots.text = f'Попаданий в тело: {self.bodyshot}'
                    self.bodyshots_percent.text = f'Процент в тело: {round(self.bodyshot / self.shoot * 100, 2)}%'

                    self.headshots_shoots.text = f'Попаданий в голову: {self.headshot}'
                    self.headshots_percent.text = f'Процент в голову: {round(self.headshot / self.shoot * 100, 2)}%'
