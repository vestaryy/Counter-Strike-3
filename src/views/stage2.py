...
class Stage2(arcade.View):
    ...
    def on_show_view(self):
        ...
        self.enemies_settings = {
            't': {'can_attack': True, 'can_move': True, 'd': 600000},
            'w': {'can_attack': True, 'can_move': True, 'd': 600000},
            'b': {'can_attack': False, 'can_move': False, 'd': 600000}
        }
        ...
        self.wall_textures = {
            ...
            'E': arcade.load_texture('../../assets/textures/sky&walls/enter.png')
    ...
    'WGWWWWWWWWWWWWWWWWW',
        ...
        self.music_player_bg = arcade.load_sound('../../assets/sounds/background/06. Night 2.mp3').play(0.3, loop=True)
    ...
    def on_update(self, delta_time):
        ...
        if (dx or dy) and self.timer_steps > 0.5:
            ...
            self.timer_steps = 0
        if 1700 < self.player.x < 1800 and 300 < self.player.y < 400 and sin(self.player.angle) > 0.7 and -0.7 < cos(self.player.angle) < 0.7 and (1700, 400) in self.block_map:
        ...
        #     self.draw_unlock = True
        elif 1700 < self.player.x < 1800 and 1600 < self.player.y < 1700 and -0.6 < sin(self.player.angle) < 0.6 and cos(self.player.angle) > 0.65 and (1800, 1600) in self.block_map:
            self.draw_unlock = True
        else:
            self.draw_unlock = False
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.E:
            if self.draw_unlock:
                if (1700, 400) in self.block_map and self.robberied_count != 9:
                    self.button_to_press.text = 'Отсутствует ключ карта'
                    arcade.load_sound('../../assets/sounds/background/error.wav').play()
                elif (1700, 400) in self.block_map and self.robberied_count == 9:
                    arcade.load_sound('../../assets/sounds/background/wall_moved.mp3').play()
                    self.button_to_press.text = 'Нажмите "Е", чтобы использовать.'
                    self.block_map.remove((1700, 400))
                elif 1700 < self.player.x < 1800 and 1600 < self.player.y < 1700 and -0.6 < sin(self.player.angle) < 0.6 and cos(self.player.angle) > 0.65 and (1800, 1600) in self.block_map:
                    self.block_map.remove((1800, 1600))
